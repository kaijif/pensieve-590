...
However, a fundamental flaw of deep neural networks is their brittleness when tasked with extrapolating far outside their training data—a phenomenon known as Out-of-Distribution (OOD) failure. Given that a pre-trained Pensieve model deployed globally would inevitably encounter network characteristics it has never seen (such as severe satellite link degradation or deep rural dead zones), assessing the degradation profile of the RL agent becomes a critical safety concern.

In this work, we take the pre-trained Pensieve model released alongside the original SIGCOMM 2017 paper and deploy it in completely unseen, extreme network environments. We ask a critical research question: \textit{How much does the RL policy degrade when network behavior falls outside the broadband envelope it was trained on, and does robustMPC degrade more or less gracefully under the same stress?}
...
\begin{itemize}
    ...
    \item We empirically demonstrate that while RL algorithms handle volatile but high-bandwidth environments effectively, their policies suffer catastrophic structural failure in extreme low-bandwidth conditions compared to mathematically bounded baselines like robustMPC.
\end{itemize}
...
This trace represents the extreme lower bound of connectivity, simulating edge-of-cell rural coverage or extreme weather-induced GEO satellite rain fade. The sustained bandwidth is capped at $0.2$ Mbps (far below the lowest 300 Kbps video bitrate option) and periodically dips to near absolute zero ($0.005$ Mbps). This condition mathematically guarantees massive rebuffering events, forcing the algorithms into a strict survival state.

\begin{figure*}[htbp]
  \centering
  \includegraphics[width=\textwidth]{test/charts/summary_bar.png}
  \caption{Normalized Average QoE across the three test conditions. While Pensieve closely matches robustMPC in Conditions 1 and 2, it suffers a massive negative QoE penalty in the extreme out-of-distribution environment of Condition 3, indicating a failure to generalize.}
  \label{fig:summary}
\end{figure*}
...
In the Satellite condition, both Pensieve and robustMPC achieved near-perfect QoE (Total Reward $\approx 198$). Surprisingly, neither algorithm registered any negative penalty from the 5-second complete network outages.

This phenomenon is explained by the fundamental mechanics of the video player. The environment specifies a maximum client buffer of 60 seconds. Because the baseline throughput is an immense 80 Mbps, both algorithms instantly download chunks and fill their buffer to the 60-second limit within the first few moments of playback. When the 3 to 5-second LEO handoff outage occurs, the video player seamlessly drains 5 seconds from its internal buffer, completely masking the outage from the user. When the connection resumes, the 80 Mbps pipe instantly refills the buffer.

\textbf{Takeaway:} Pensieve generalizes perfectly to high-throughput environments with transient dropouts. However, this reveals that micro-outages do not effectively test ABR policy degradation, as the 60-second buffer acts as a perfect abstraction layer against network volatility.
...
In the Bursty Wi-Fi condition, the environment oscillates violently between 1 Mbps and 5 Mbps. Under these conditions, Pensieve slightly outperformed or directly matched robustMPC.
...
\begin{figure}[htbp]
  ...
  \caption{Bitrate choices over sequential video chunks for a single representative trace in Condition 3 (Degraded). robustMPC immediately identifies the capacity deficit and locks onto the minimum 300 Kbps tier. Pensieve wildly extrapolates, attempting to pull 4300 Kbps chunks over a 0.2 Mbps connection, resulting in catastrophic rebuffering.}
  \label{fig:qualitative3}
\end{figure}

\subsection{Condition 3: Catastrophic Extrapolation Failure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{test/charts/cdf_cond3.png}
  \caption{CDF of Total QoE for Condition 3 (Degraded). robustMPC significantly bounds the negative QoE penalty compared to Pensieve's massive failure tail.}
  \label{fig:cdf3}
\end{figure}

Condition 3 provides the critical answer to our research question. When exposed to an extreme, sustained degraded network (sub-0.2 Mbps) far outside its standard training distribution, the Pensieve RL policy broke entirely.

As shown in Figure \ref{fig:summary}, robustMPC maintained an average QoE of approximately -2,242. While severely negative (due to mathematically unavoidable rebuffering), it represents a controlled failure state. By contrast, Pensieve scored a disastrous -32,991.

Figure \ref{fig:qualitative3} illustrates the root cause of this massive discrepancy. The robustMPC algorithm behaved exactly as designed: recognizing that the harmonic mean of the throughput was consistently below 0.2 Mbps, its internal optimizer recognized that any choice above the minimum 300 Kbps tier would heavily penalize the objective function. It safely locked the video player into the lowest possible quality, finishing the 48-chunk video in ~800 real-world seconds.

Pensieve, operating completely out-of-distribution, suffered catastrophic policy failure. Confronted with state vectors it had never mapped, it incorrectly activated neurons suggesting high network capacity. As seen in Figure \ref{fig:qualitative3}, Pensieve wildly thrashed, repeatedly requesting impossibly high bitrates of 2850 Kbps and 4300 Kbps. Because the network could only supply 0.2 Mbps, attempting to download a 4300 Kbps chunk caused massive, crippling rebuffering delays. In extreme cases, it took Pensieve over 10,700 seconds to finish downloading the exact same 300-second video that robustMPC downloaded in 800 seconds.
...
However, when pushed into a severe low-bandwidth extreme outside its training envelope, the black-box RL policy fails to extrapolate the basic survival mechanic of staying at the lowest bitrate. Conversely, robustMPC degrades significantly less in these extreme OOD scenarios. Because robustMPC explicitly calculates future buffer states using mathematical formulas rather than pattern matching, it maintains its structural safety guarantees regardless of the input distribution.

While Reinforcement Learning achieves higher peak performance in familiar distributions, mathematical prediction algorithms provide a far superior, safer worst-case lower bound in unfamiliar extremes. Future work in RL-based ABR must address these extrapolation boundaries, potentially utilizing hybrid approaches that enforce MPC safety bounds when deep learning policies output high-variance or low-confidence actions.
